void T4_3_2() // Este macro realiza un histograma comparando el conteo de ocurrencias por PMT para el evento 0 y 999
{
...
for(Int_t i=0; i<nChannelsInfo; i++) // ciclo for para eventos
   {        // se llena el histograma
        histo1->AddBinContent(channelIDInfo[i], (channelCountInfo[i])/24.0);
   }
...
for(Int_t i=0; i<nChannelsInfo; i++)
   {        // se llena el histograma
        histo2->AddBinContent(channelIDInfo[i], (channelCountInfo[i])/24.0);
   }
